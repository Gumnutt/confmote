...
const reactionMapping = (original) => {
  ...
  Object.keys(reactionMap).map((reactionKey) => {
    reactionMap[reactionKey].action = emojiEvent(reactionMap[reactionKey].emoji, reactionKey)
  })
  return reactionMap
}
...
const actionMapping = (original) => {
  ...
  Object.keys(toggleMap).map((toggleKey) => {
    ...
    toggleMap[toggleKey].label = toggleMap[toggleKey].label.replace("React with", "Toggle")
